...
def export_iocs_to_pdf(file_path, hashes, strings, vt_data, mitre_hits, imports=[]):
    pdf_path = os.path.splitext(file_path)[0] + "_iocs.pdf"
    pdf = FPDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 10, "MalVista - Malware IOC Report", ln=True)
    ...
    if imports:
        ...
        for entry in imports:
            if isinstance(entry, tuple):
                dll, funcs = entry
                pdf.multi_cell(0, 8, f"{dll}: {', '.join(funcs[:5])}")
            else:
                pdf.multi_cell(0, 8, f"Import parsing error: {entry}")
    ...
    if mitre_hits:
        ...
        pdf.multi_cell(0, 10, "No techniques matched.")
